package com.jpa.relationmapping;

import java.io.Serializable;
import java.util.Date;
import java.util.List;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.EntityManager;
import javax.persistence.EntityManagerFactory;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.JoinColumn;
import javax.persistence.JoinTable;
import javax.persistence.ManyToOne;
import javax.persistence.Persistence;
import javax.persistence.Table;
import javax.persistence.Temporal;
import javax.persistence.TemporalType;

/**
 * Demonstrate Many To One Relationship With Mapping Multiple Join
 * Hibernate modeled the mapping using separate Table to join both of Entities
 * 
 * 	1.	Observe how the table is drop and create back define in META-INF/persistence.xml 
 * 			<property name="javax.persistence.schema-generation.database.action" value="drop-and-create"/>
 * 
 *  	Observe data is initialize through insert statement in preloaded-onlineshop-data.sql 
 *  		 <property name="javax.persistence.sql-load-script-source" value="META-INF/preload-query.sql"/>
 *  	insert statements are structure in one line per statement
 *  	only triggered if the schema generation action is set to drop and create
 * 
 *  2.	Observe how the entities classes are defining Table name with annotation @Table(name="")
 *  
 *  3.	Look how the GeneratedValue is define in entity class with different strategy assigning primary key generator
 *  		> AUTO 		: uses default sequence (hibrnate_sequence)
 *  		> IDENTITY  : uses auto increment of each table
 *  		> SEQUENCE  : uses definied sequence
 *  		> TABLE 	: uses definied table object as sequence generator
 *  
 *  4.	Defined how to defined custom Column definition with @Column Annotation
 *  		check how the column name can be altered or adjust,
 *  		Column Field Type, Column length
 *  		specify the Floating point precision (max digit) and scale ( n digit after decimal ) 
 *  
 *  5.	Demonstrate How to Date Time Field stored in Database
 *  		Normal / DEFAULT / Temporal (TemporalType.TIMESTAMP) --> Store both Date and Time Info
 *  		Temporal (TemporalType.DATE) --> Store Only Date without Time info
 *  		Temporal (TemporalType.TIME) --> Store Only Time without Date info
 *  
 *  6. Observe Annotation @ManyToOne is defined on Collections method or fields 
 *  
 *  7. Observe Annotation @JoinTable is defined on 
 *     
 *  
 *  Mapping : 
 *  	my_products (order_id) --< my_order (order_id)
 *		
 *
 */
public class Mapping14ManyToOneMappingMultipleJoin {

	/**
	 * 

Hibernate: 
    
    create table my_9th_products (
       id integer not null auto_increment,
        name varchar(255),
        quantity integer,
        primary key (id)
    ) engine=MyISAM
           
    
    create table my_9th_mapping (
       order_id integer,
        product_id integer not null,
        primary key (product_id)
    ) engine=MyISAM
    
    alter table my_9th_mapping 
       add constraint FKno2pe3pmdnujg9flwbb3mo2b 
       foreign key (order_id) 
       references my_9th_order (id)
    
    alter table my_9th_mapping 
       add constraint FK8u7q06k5wlylrs12d4jr6mhw7 
       foreign key (product_id) 
       references my_9th_products (id)         
       
	 *
	 */
	@Entity(name="MyProducts9")
	@Table(name="my_9th_products")
	public static class Product implements Serializable {
		
		private static final long serialVersionUID = 1L;
		
		@Id
		@GeneratedValue(strategy = GenerationType.IDENTITY)
		private Integer id;
		
		private String name;
		
		private Integer quantity;
		
		@ManyToOne
		@JoinTable(
				name = "my_9th_mapping",
				joinColumns =  { @JoinColumn(name = "product_id", referencedColumnName = "id") },
				inverseJoinColumns = { @JoinColumn(name = "order_id", referencedColumnName = "id")}
		)
		private Order order;

		public Product() {
		}

		public Product(Order order, String name, Integer quantity) {
			this.order = order;
			this.name = name;
			this.quantity = quantity;
		}

		public Integer getId() {
			return id;
		}

		public void setId(Integer id) {
			this.id = id;
		}

		public String getName() {
			return name;
		}

		public void setName(String name) {
			this.name = name;
		}

		public Integer getQuantity() {
			return quantity;
		}

		public void setQuantity(Integer quantity) {
			this.quantity = quantity;
		}
		
		public Order getOrder() {
			return order;
		}

		public void setOrder(Order order) {
			this.order = order;
		}

		@Override
		public String toString() {
			return String.format("{ %d, %s, %d }", this.id, this.name, this.quantity);
		}

	}
	
	/**
	 * 

 Hibernate: 
    
    create table my_9th_order (
       id integer not null auto_increment,
        order_date date,
        primary key (id)
    ) engine=MyISAM
        
        
    alter table my_9th_order 
       add constraint UK_tcqs60a1twao1ovig0dvucr0q unique (order_date)
               
	 *
	 */
	@Entity(name = "MyOrder9")
	@Table(name = "my_9th_order")
	public static class Order implements Serializable {

		private static final long serialVersionUID = 1L;

		@Id
		@GeneratedValue(strategy = GenerationType.IDENTITY)
		private Integer id;
		
		@Column(name = "order_date")
		@Temporal(TemporalType.DATE)
		private Date orderDate;

		public Order() {
		}

		public Order(List<Product> products, Date orderDate) {
			super();
			this.orderDate = orderDate;
		}

		public Integer getId() {
			return id;
		}

		public void setId(Integer id) {
			this.id = id;
		}

		public Date getOrderDate() {
			return orderDate;
		}

		public void setOrderDate(Date orderDate) {
			this.orderDate = orderDate;
		}

		@Override
		public String toString() {
			return String.format("{ %d, %s }", this.id, this.orderDate);
		}
		
	}
	
	private EntityManagerFactory factory;
	private EntityManager em;
	
	public void runMain() {
		factory = Persistence.createEntityManagerFactory("OnlineShoopingDB_Unit");
		em = factory.createEntityManager();

		try {
			System.out.println("retrieve data");
			
			/**
			 * 
			 * Observe How the Query is being generated 
			 * 

Hibernate: 
    select
        mapping14m0_.id as id1_37_0_,
        mapping14m0_.name as name2_37_0_,
        mapping14m0_.quantity as quantity3_37_0_,
        mapping14m0_1_.order_id as order_id1_35_0_,
        mapping14m1_.id as id1_36_1_,
        mapping14m1_.order_date as order_da2_36_1_ 
    from
        my_9th_products mapping14m0_ 
    left outer join
        my_9th_mapping mapping14m0_1_ 
            on mapping14m0_.id=mapping14m0_1_.product_id 
    left outer join
        my_9th_order mapping14m1_ 
            on mapping14m0_1_.order_id=mapping14m1_.id 
    where
        mapping14m0_.id=?
                               
			 * 
			 */
			
			Product productOne = em.find(Product.class, 1);
			System.out.println(productOne);
			System.out.println(productOne.getOrder());
			
			Product productFive = em.find(Product.class, 5);
			System.out.println(productFive);
			System.out.println(productFive.getOrder());
			
			
			Order orderTwo = em.find(Order.class, 12);
			System.out.println(orderTwo);
			
			/**
			 * 

Hibernate: 
    select
        mapping14m0_.id as id1_37_,
        mapping14m0_.name as name2_37_,
        mapping14m0_.quantity as quantity3_37_,
        mapping14m0_1_.order_id as order_id1_35_ 
    from
        my_9th_products mapping14m0_ 
    left outer join
        my_9th_mapping mapping14m0_1_ 
            on mapping14m0_.id=mapping14m0_1_.product_id
            
    select
        mapping14m0_.id as id1_36_0_,
        mapping14m0_.order_date as order_da2_36_0_ 
    from
        my_9th_order mapping14m0_ 
    where
        mapping14m0_.id=?        
        
			 * 			
			 */
			
			List<Product> products = em.createQuery("SELECT a FROM MyProducts9 a", Product.class).getResultList();
			System.out.println(products);
			
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
			em.close();
			factory.close();
		}
	}
	
	
	public static void main(String[] args) {
		new Mapping14ManyToOneMappingMultipleJoin().runMain();
	}

}
